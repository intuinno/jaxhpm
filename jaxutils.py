...
class Optimizer(nj.Module):
    ...
    def __init__(self, lr):
        ...
        if self.scaling:
            self.chain = optax.apply_if_finite(self.chain, max_consecutive_errors=1000)
            ...
            metrics.update(
                {
                    ...
                    for k, v in dict(
                        ...
                        param_abs_mean=jnp.stack(
                            [jnp.abs(x).mean() for x in ps]
                        ).mean(),
                        param_norm=optax.global_norm(ps),
                        update_abs_max=jnp.stack([jnp.abs(x).max() for x in us]).max(),
                        update_abs_mean=jnp.stack(
                            [jnp.abs(x).mean() for x in us]
                        ).mean(),
                ...
                metrics.update(
                    {
                        f"{k}/{name}": v
                        for k, v in dict(
                            scale_abs_max=jnp.stack(
                                [jnp.abs(x).max() for x in sc]
                            ).max(),
                            scale_abs_min=jnp.stack(
                                [jnp.abs(x).min() for x in sc]
                            ).min(),
                            scale_abs_mean=jnp.stack(
                                [jnp.abs(x).mean() for x in sc]
                            ).mean(),
...
class SlowUpdater(nj.Module):
    ...
    def __call__(self):
        ...
        ema = treemap(lambda s, d: mix * s + (1 - mix) * d, params, self.dst.find())
