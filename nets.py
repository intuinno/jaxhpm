...
class RSSM(nj.Module):
    ...
    def loss(self, outs, free=1.0):
        ...
        metrics.update(jaxutils.tensorstats(self._dist(prior).entropy(), "prior_ent"))
        metrics.update(jaxutils.tensorstats(self._dist(post).entropy(), "post_ent"))
    ...
    def _gru(self, deter, stoch, action):
        ...
        if self.cell == "gru":
            ...
            for i in range(self.dynlayers):
                ...
                x = get_act(self.act)(
                    jnp.concatenate(
                        [
                            ...
                            self.get(f"dyngru{i}norm2", Norm, self.norm)(x),
                        ],
                        -1,
                    )
                )
                x = self.get(
                    f"dyngru{i}core", Linear, 3 * deters[i].shape[-1], **self.kw
                )(x)
                reset, cand, update = jnp.split(x, 3, -1)
                reset = jax.nn.sigmoid(reset)
                cand = jnp.tanh(reset * cand)
                ...
                x = self.get(f"dynmlp{i}up", Linear, deters[i].shape[-1], **self.kw)(x)
    ...
    def __call__(self, lat, bdims=2):
        ...
        if self.veckeys:
            ...
            for k in self.veckeys:
                dist = (
                    ...
                    if self.spaces[k].discrete
                    else dict(dist=self.vecdist)
                )
                k = k.replace("/", "_")
        ...
        if self.imgkeys:
            ...
            for i, depth in reversed(list(enumerate(self.depths[:-1]))):
                x = self.get(
                    f"conv{i}", Conv2D, depth, self.kernel, 2, **kw, transp=True
                )(x)
            outkw = dict(**self.kw, outscale=self.outscale, transp=True)
            stride = 1 if self.debug_outer else 2
            x = self.get("imgout", Conv2D, self.imgdep, self.kernel, stride, **outkw)(x)
...
class MLP(nj.Module):
    ...
    def __call__(self, inputs, bdims=2, training=False):
        ...
        if self.shape is None:
            ...
            return {k: self._out(k, v, self.dist[k], x) for k, v in self.shape.items()}
        else:
...
class BlockLinear(nj.Module):
    ...
    def __init__(self, units, groups):
        ...
        self._winit = Initializer(
            self.winit, self.outscale, self.fan, self.dtype, block_fans=self.block_fans
        )
        self._binit = Initializer("zeros", 1.0, self.fan, self.dtype)
        if self.block_norm:
            self._norm = [Norm(self.norm, name=f"norm{i}") for i in range(self.groups)]
        else:
            self._norm = Norm(self.norm, name="norm")
...
class Initializer:
    ...
    def __init__(
        self, dist="normal", scale=1.0, fan="in", dtype="default", block_fans=False
    ):
        self.dist = dist
        self.scale = scale
        self.fan = fan
    ...
    def __call__(self, shape, fan_shape=None):
        ...
        if self.dist == "zeros":
            ...
            value = jax.random.truncated_normal(nj.seed(), -2, 2, (2, *shape), realdt)
